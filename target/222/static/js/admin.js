...
$(function() {
    ...
    var systemname = $.cookie('zheng-upms-systemname') || 'admin';
    var systemtitle = $.cookie('zheng-upms-systemtitle') || '学工部贫困生信息管理端';
	...
	addTab: function(title, url) {
		...
		if ($('#tab_' + index).length == 0) {
            ...
            var tab = '<li id="tab_' + index +'" data-index="' + index + '" class="cur">' +
				'<span class="waves-effect waves-light">' + title + '</span>' +
				'<i class="waves-effect waves-light zmdi zmdi-close" onclick="Tab.closeTab($(\'#tab_' + index +'\'));">' +
				'</i>' +
				'</li>';
			...
			var iframe = '<div id="iframe_' + index + '" class="iframe cur">' +
				'<iframe class="tab_iframe" src="' + url + '" width="100%" frameborder="0" scrolling="auto" onload="changeFrameHeight(this)"></iframe></div>';
